                                ...
                                {block name="field"}
                                    ...
                                    {block name="input"}
                                        ...
                                        {elseif $input.type == 'textbutton'}
                                            ...
                                            {/if}
                                        {elseif $input.type == 'configuration'}
                                            <div class="configuration">
                                                <input type="hidden" name="configurationDataInput" id="configurationDataInput" />
                                                <script type="application/javascript">
                                                    {if isset($fields_value[$input.name])}
                                                    {*{$fields_value|print_r}*}
                                                    {*{$input.name}*}
                                                    {assign var=initial_configuration value=$fields_value[$input.name]|rtrim:'"'|trim:'"'}

                                                    {if $initial_configuration != '' && $initial_configuration != 'null'}
                                                    var initial_configuration = {$initial_configuration};
                                                    {else}
                                                    {literal}
                                                    ...
                                                    var fieldset_type = {$input.types_json};
                                                    var tradAddField = '{l s='add a field' js=1}';
                                                    var tradFieldName = '{l s='field name' js=1}';
                                                    var tradFieldDesc = '{l s='field description' js=1}';
                                                    var tradRemove = '{l s='Remove' js=1}';
                                                    var fieldset_parent = $('#fieldsetContainer').parents('.form-group');
                                                    {literal}
                                                    var fieldsetData = {};
                                                    {/literal}
                                                ...
                                                <a id="addConfigFielset" class="btn btn-default" href="#">
                                                    <i class="icon-add"> {l s='add a fieldset'}</i>
                                                </a>
                                                <input placeholder="{l s='fieldset name'}" style="display: inline-block; width: 25%" name="fieldsetName" type="text" />
                                        ...
                                        {elseif $input.type == 'entities'}
                                            <div class="entities">
                                                <input type="hidden" name="entityDataInput" id="entityDataInput" />
                                                <script type="application/javascript">
                                                    {if isset($fields_value[$input.name])}
                                                        {assign var=initial_entities value=$fields_value[$input.name]|rtrim:'"'|trim:'"'}
                                                        {if $initial_entities != ''}
                                                            var initial_entities = {$initial_entities};
                                                        {else}
                                                            {literal}
                                                    ...
                                                    var entities_type = {$input.types_json};
                                                    var boolean_options = {$input.boolean_options_json}
                                                    var has_fixtures = {if $has_fixtures}true{else}false{/if};
                                                    var tradAddField = '{l s='add a field' js=1}';
                                                    var tradFieldName = '{l s='field name' js=1}';
                                                    var tradFieldDesc = '{l s='field description' js=1}';
                                                    var tradRemove = '{l s='Remove' js=1}';
                                                    var entities_parent = $('#entitiesContainer').parents('.form-group');
                                                    {literal}
                                                    var entityData = {};
                                                    {/literal}
                                                ...
                                                <a id="addEntities" class="btn btn-default" href="#">
                                                    <i class="icon-add"> {l s='add an entity'}</i>
                                                </a>
                                                <input placeholder="{l s='entity class name'}" style="display: inline-block; width: 25%" name="entityName" type="text" />
                                                <input placeholder="{l s='entity name'}" style="display: inline-block; width: 25%" name="entityNameFront" type="text" />
                                                ...
                                                {foreach $input.values.query as $value}
                                                    ...
                                                    <div class="checkbox{if isset($input.expand) && $input.expand.default == 'show'} hidden {/if}">
                                                        <label for="{$id_checkbox}">
                                                            <input class="optionable" type="checkbox"
                                                                   name="{$id_checkbox}"
                                                                   id="{$id_checkbox}"
                                                        ...
                                                        <div class="options" style="display:none">
                                                            {if $has_superblock}
                                                            <label for="superblock">
                                                                {assign var=id_hook_superblock value=$id_checkbox|cat:'_superblock'}
                                                                <input type="text"
                                                                       name="{$id_checkbox}_superblock"
                                                                       id="{$id_checkbox}_superblock"
                                                                       {if isset($fields_value[$id_hook_superblock])}value="{$fields_value[$id_hook_superblock]}"{/if}
                                                                       class="{if isset($input.class)}{$input.class}{/if}"
                                                                       {if isset($value.val)}value="{$value.val|escape:'html':'UTF-8'}"{/if}
                                                                 />
                                                                SuperBlock
                                                            ...
                                                            {if $has_superblock}
                                                            <label for="superblock">
                                                                {assign var=id_hook_menu value=$id_checkbox|cat:'_menu'}
                                                                <input type="text"
                                                                       name="{$id_checkbox}_menu"
                                                                       id="{$id_checkbox}_menu"
                                                                       {if isset($fields_value[$id_hook_menu])}value="{$fields_value[$id_hook_menu]}"{/if}
                                                                       class="{if isset($input.class)}{$input.class}{/if}"
                                                                       {if isset($value.val)}value="{$value.val|escape:'html':'UTF-8'}"{/if}
                                                                 />
                                                                Menu
            ...
            {block name="footer"}
                {if isset($fieldset['form']['submit']) || isset($fieldset['form']['buttons'])}
                    <div class="panel-footer">
                        ...
                        <button
                                type="submit"
                                id="generateAndInstall"
                                name="generateAndInstall"
                                class="btn btn-default pull-right"
                                >
                            <i class="icon-AdminAttributeGenerator"></i> {l s='Generate and install'}
                        </button>
