...
<form id="{$table}_form" class="defaultForm {$name_controller}" action="{$current}&{if !empty($submit_action)}{$submit_action}=1{/if}&token={$token}" method="post" enctype="multipart/form-data" {if isset($style)}style="{$style}"{/if}>
	...
	{foreach $fields as $f => $fieldset}
		<fieldset id="fieldset_{$f}" {if isset($fieldset.class)}class="type_box box-{$fieldset.class}"{/if}>
			{foreach $fieldset.form as $key => $field}
				...
				{elseif $key == 'input'}
					{foreach $field as $input}
                        ...
                        {if isset($input.container_class)}<div class="field-{$input.container_class}">{/if}
                        {if $input.type == 'menu_item_positin'}
                            {$position_selector}
						...
						{else}
							...
							{/if}
						{/if}
                        {if isset($input.container_class)}</div>{/if}
					{/foreach}
